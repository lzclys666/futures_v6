...
"""
EC_欧线期货跨期价差.py
因子: EC_SPD_01 = 集运指数主力-次主力价差（1个合约月后）
因子: EC_SPD_03 = 集运指数主力-远月价差（3个合约月后）
因子: EC_SPD_05 = 集运指数主力-超远月价差（5个合约月后）
...
SYMBOL = "EC"
# EC 交易月份: 02, 04, 06, 08, 10, 12（偶数月）
EC_MONTHS = [2, 4, 6, 8, 10, 12]
BOUNDS = (-5000, 5000)
...
def main():
    ...
    if pub_date is None:
        print("-- 非交易日"); return
    print(f"=== EC跨期价差 === pub={pub_date} obs={obs_date}")
    ...
    if len(contracts) < 2:
        ...
        for fcode, _ in FACTOR_DEFS:
            save_l4_fallback(fcode, SYMBOL, pub_date, obs_date, extra_msg="集运指数跨期价差")
        return
