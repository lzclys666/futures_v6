...
"""
J_焦炭期货近远月价差.py
因子: J_SPD_NEAR_FAR = 焦炭期货近远月价差（元/吨）

公式: J_SPD_NEAR_FAR = 近月合约收盘价 - 远月合约收盘价

当前状态: ✅正常
- L1: AKShare futures_main_sina(symbol='J{YYMM}') — 逐合约获取收盘价
  自动枚举当前年份所有合约月份，取最近两个活跃合约计算价差
- L4: save_l4_fallback() DB历史最新值回补

已验证: J2605=1696.5, J2606=1740.5, 价差=-44.0, obs=2026-04-30
"""
import sys, os
...
FCODE = "J_SPD_NEAR_FAR"
SYM = "J"
BOUNDS = (-500, 500)  # 价差合理范围（元/吨）
BACKOFF_DAYS = 15  # 回溯天数
...
def main():
    ...
    print("[L1 FAIL] 合约价差计算失败, trying L4...")
