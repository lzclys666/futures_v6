...
"""
NR_橡胶合约间价差.py
因子: NR_SPD_CONTRACT = 20号胶合约间价差

公式: NR_SPD_CONTRACT = NR0主力收盘价 - NR2次主力收盘价（元/吨）

当前状态: [⚠️待修复]
- L1: AKShare futures_main_sina(symbol="NR0"/"NR2") — 新浪期货主力/次主力
- L2: 无备源
- L3: 无备源
...
FACTOR_CODE = "NR_SPD_CONTRACT"
SYMBOL = "NR"
BOUNDS = (-500, 500)


def fetch():
    df0 = ak.futures_main_sina(symbol="NR0")
    df2 = ak.futures_main_sina(symbol="NR2")
    if df0.empty or df2.empty:
        raise ValueError("AKShare无NR数据")
...
def main():
    ...
    save_to_db(FACTOR_CODE, SYMBOL, pub_date, obs_date, raw_value,
               source_confidence=1.0, source='AKShare_Sina_NR0_NR2')
