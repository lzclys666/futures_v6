...
def main():
    try:
        ...
        if latest is not None:
            print("[L4FB] %s=%.2f" % (FACTOR_CODE, latest))
            return
    ...
    pub_date = date.today()
    save_to_db(FACTOR_CODE, SYMBOL, pub_date, obs_date, raw_value, source="akshare_sina", source_confidence=1.0)
