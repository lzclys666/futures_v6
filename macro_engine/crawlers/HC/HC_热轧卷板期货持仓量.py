...
公式: 无（直接采集AKShare HC0主力合约持仓量）

当前状态: [正常]
- L1: AKShare futures_main_sina(HC0) 主力合约持仓量
...
def main():
    ...
    try:
        ...
        if not (BOUNDS[0] <= oi <= BOUNDS[1]):
            sys.stdout.write(f"[WARN] {FACTOR_CODE}={oi} out of {BOUNDS}\n")
