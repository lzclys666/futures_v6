...
"""
SN_沪锡期现基差.py
因子: SN_SPD_BASIS = 沪锡期现基差（元/吨）

公式: 现货价 - 期货主力合约收盘价

当前状态: [✅正常]
- L1: AKShare futures_spot_price(date, vars_list=["SN"]) — 直接返回基差
- L2: 手动计算 = 现货价 - futures_main_sina("SN0")收盘价
...
SPOT_SYM_ALTERNATIVES = ["SN", "sn", "锡", "沪锡"]
...
def fetch_l2(obs_date):
    ...
    futures_price = None
    try:
        print("[L2] futures_main_sina('SN0')...")
