...
"""
SN_沪锡期货净持仓.py
因子: SN_POS_NET = 上期所沪锡前20净持仓（手）

公式: sum(long_open_interest across all SN contracts top 20) - sum(short_open_interest across all SN contracts top 20)
...
def fetch(obs_date):
    """获取沪锡前20净持仓，自动回退最多BACKOFF_DAYS个自然日找最近交易日"""
    for backoff in range(BACKOFF_DAYS):
        ...
        sn_keys = [k for k in result if k.startswith('sn')]
        if not sn_keys:
            print(f"  [backoff {backoff}] {date_str}: no SN keys in result")
            continue
    ...
    raise ValueError(f"get_shfe_rank_table failed after {BACKOFF_DAYS} backoff attempts — no SN data on any recent trading day")
