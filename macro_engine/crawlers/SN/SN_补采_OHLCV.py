...
"""
SN_补采_OHLCV.py
用途: 创建 sn_futures_ohlcv 表并补采历史 OHLCV 数据

数据源: AKShare futures_zh_daily_sina(symbol='sn0')
...
def create_table():
    """创建 sn_futures_ohlcv 表"""
    conn = sqlite3.connect(DB_PATH)
    conn.execute("""
        CREATE TABLE IF NOT EXISTS sn_futures_ohlcv (
            pub_date TEXT,
            obs_date TEXT,
            contract TEXT,
            trade_date TEXT,
    ...
    conn.commit()
    conn.close()
    print("[OK] sn_futures_ohlcv 表已创建/确认存在")


def backfill():
    """从 AKShare 补采 sn0 主力合约历史 OHLCV"""
    print("[L1] AKShare futures_zh_daily_sina(symbol='sn0')...")
    ...
    for _, row in df.iterrows():
        ...
        try:
            conn.execute("""
                INSERT OR REPLACE INTO sn_futures_ohlcv
    ...
    conn.commit()
    conn.close()
    print(f"[OK] 补采完成: {count} 条记录写入 sn_futures_ohlcv")
    return count
