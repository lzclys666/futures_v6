...
"""
SN_沪锡期货仓单.py
因子: SN_STK_WARRANT = 上期所沪锡仓单库存（吨）

公式: 数据采集（无独立计算公式）

当前状态: [✅正常]
- L1: AKShare futures_shfe_warehouse_receipt(date) — SHFE仓单数据 → 可能JSONDecodeError(SHFE反爬)
- L2: AKShare futures_inventory_em(symbol="沪锡") — 东方财富沪锡库存
...
FCODE = "SN_STK_WARRANT"
SYM = "SN"
BOUNDS = (0, 50000)  # 仓单合理范围（吨）
BACKOFF_DAYS = 15
...
def fetch_l1(obs_date):
    ...
    for backoff in range(BACKOFF_DAYS):
        ...
        try:
            ...
            # Find SN-related column
            sn_col = None
            for c in df.columns:
                cs = str(c).lower()
                if 'sn' in cs or '锡' in cs:
                    sn_col = c
                    break
            if sn_col is None:
                for c in df.columns:
                    if any(x in str(c) for x in ['仓单', '库存', '吨']):
                        sn_col = c
                        break
            if sn_col is None and len(df.columns) >= 2:
                sn_col = df.columns[-1]

            if sn_col:
                val = df.iloc[-1][sn_col]
