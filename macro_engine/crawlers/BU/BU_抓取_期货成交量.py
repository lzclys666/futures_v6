...
"""
BU_沥青期货成交量.py
因子: BU_FUT_VOL = 沥青期货成交量（手）

公式: 数据采集（取主力合约成交量）

当前状态: [✅正常]
- L1: AKShare futures_main_sina(symbol='BU0')，取成交量字段
- L2: 无备选源
...
FACTOR_CODE = "BU_FUT_VOL"
SYMBOL = "BU"
BOUNDS = (100000.0, 2000000.0)  # 手


def fetch():
    """L1: AKShare BU0沥青期货日行情，取成交量"""
    print("[L1] AKShare futures_main_sina(symbol='BU0')...")
    df = ak.futures_main_sina(symbol="BU0")
    if df is None or df.empty:
        raise ValueError("no data")
    latest = df.sort_values("日期").iloc[-1]
    vol = float(latest.get("成交量") or 0)
    if vol <= 0:
        raise ValueError(f"成交量异常: {vol}")
