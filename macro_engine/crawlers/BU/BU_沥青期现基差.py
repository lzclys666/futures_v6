    ...
    try:
        ...
        if df is not None and len(df) > 0:
            ...
            if close is not None and not pd.isna(close):
                close = float(close)
                print(f"[L1] BU0 settle: {close}")
                return close
    except Exception as e:
        print(f"[L1] BU0 fut price failed: {e}")
    return None
...
if __name__ == "__main__":
    ...
    if spot_price is not None and fut_price is not None:
        ...
        save_to_db("BU_SPOT_PRICE", SYMBOL, pub_date, actual_date, spot_price, source_confidence=0.9, source='akshare_futures_spot_price')
    else:
