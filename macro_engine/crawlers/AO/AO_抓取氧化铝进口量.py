...
"""
AO_抓取氧化铝进口量.py
因子: AO_IMP_ALUMINA = 中国氧化铝进口量（万吨/月）
...
def fetch():
    ...
    try:
        # 尝试通用海关数据接口
        df = ak.macro_china_imports(index="氧化铝")
    ...
    try:
        df = ak.macro_china_trade_balance(index="氧化铝")
        if df is not None and len(df) > 0:
...
def main():
    ...
    if save_l4_fallback(FACTOR_CODE, SYMBOL, pub_date, obs_date, extra_msg="(氧化铝进口量)"):
        pass
    else:
