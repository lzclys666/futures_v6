#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Y_run_all.py - 棕榈油数据采集（subprocess模式）"""
...
def run_all():
    ...
    log_file = LOG_DIR / f"{now.strftime('%Y-%m-%d')}_Y.log"

    print("=" * 50)
    print(f"Y Data Collection @ {now}")
    ...
    with open(log_file, "a", encoding="utf-8") as log:
        ...
        print(f"\n{'='*50}")
        print(f"Y Done  {duration:.1f}s  {success_count}/{len(scripts)}")
        if failures:
            for n, r in failures:
                print(f"  - {n}: {r}")
        else:
