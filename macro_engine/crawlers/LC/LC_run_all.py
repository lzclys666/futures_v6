...
def main():
    ...
    sep = "=" * 50
    print(sep)
    print(f"LC @ {time.strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    print(sep)
    total = ok + fail + skip
    print(f"LC Done {ok+fail+skip}/{total}  OK={ok} FAIL={fail} SKIP={skip}")
    print(sep)
    ...
    with open(log_file, 'a', encoding='utf-8') as f:
        f.write(f"LC @ {datetime.datetime.now()} | pub={pub_date} obs={obs_date}\n")
