...
"""
LC_碳酸锂期货持仓量变化.py
因子: LC_POS_OI = 碳酸锂期货主力合约持仓量
因子: LC_POS_CHANGE = 碳酸锂期货持仓量日变化

公式: LC_POS_CHANGE = LC_POS_OI(today) - LC_POS_OI(yesterday)

当前状态: [✅正常]
- L1: AKShare futures_main_sina(symbol='LC0') 持仓量列，source_confidence=1.0
...
SYMBOL = "LC"
FCODE_OI = "LC_POS_OI"
FCODE_CHANGE = "LC_POS_CHANGE"
BOUNDS_OI = (100, 1000000)
BOUNDS_CHANGE = (-600000, 600000)


def fetch_oi():
    """L1: 从AKShare获取主力合约持仓量"""
    print("[L1] AKShare futures_main_sina(symbol='LC0')...")
...
def main():
    ...
    try:
        ...
        print(f"[L1 FAIL] {e}")
        save_l4_fallback(FCODE_OI, SYMBOL, pub_date, obs_date, extra_msg="碳酸锂持仓量")
        save_l4_fallback(FCODE_CHANGE, SYMBOL, pub_date, obs_date, extra_msg="碳酸锂持仓量变化")
        return
