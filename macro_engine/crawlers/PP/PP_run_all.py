#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""PP_run_all.py - 聚丙烯数据采集（subprocess模式）"""
...
scripts = ["PP_聚丙烯期货收盘价.py", "PP_聚丙烯期货持仓量.py"]

def run_all():
    now = datetime.datetime.now()
    log_file = LOG_DIR / f"{now.strftime('%Y-%m-%d')}_PP.log"

    print("=" * 50)
    print(f"PP Data Collection @ {now}")
    ...
    with open(log_file, "a", encoding="utf-8") as log:
        ...
        print(f"\n{'='*50}")
        print(f"PP Done  {duration:.1f}s  {success_count}/{len(scripts)}")
        if failures:
            for n, r in failures:
                print(f"  - {n}: {r}")
        else:
