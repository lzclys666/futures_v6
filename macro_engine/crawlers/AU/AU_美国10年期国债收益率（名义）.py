...
当前状态: [✅正常]
- L1: AKShare bond_zh_us_rate()，source_confidence=1.0
- L2: FRED DGS1010（美国10年期国债名义收益率），source_confidence=0.9
...
def fetch_l2():
    """L2: FRED DGS10（美国10年期国债名义收益率）"""
    print("[L2] FRED DGS10...")
    today = datetime.now().strftime('%Y-%m-%d')
    url = f"https://fred.stlouisfed.org/graph/fredgraph.csv?id=DGS10&vintage_date={today}"
    r_text, err = fetch_url(url, timeout=15)
    if err:
        raise ValueError(f"FRED DGS10 failed: {err}")
    ...
    raise ValueError("FRED DGS10: no valid data")
