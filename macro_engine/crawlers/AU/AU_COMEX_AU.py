...
"""
AU_COMEX_AU.py
因子: AU_COMEX_AU = COMEX黄金期货主力收盘价（美元/盎司）

公式: 数据采集（无独立计算公式）

当前状态: ✅正常
- 数据源: AKShare futures_foreign_hist(symbol='GC')，L1权威
- 采集逻辑: 取'close'列最新一行
- obs_date: 数据日期
- bounds: [1000.0, 4000.0]美元/盎司（历史区间）
...
FACTOR_CODE = "AU_COMEX_AU"
SYMBOL = "AU"
BOUNDS = (1000.0, 4000.0)


def fetch():
    """L1: AKShare COMEX黄金期货（GC）"""
    print_enc("[L1] AKShare futures_foreign_hist(symbol='GC')...")
    ...
    print_enc(f"[L1] COMEX_AU=${raw_value} obs={obs_date}")
...
if __name__ == "__main__":
    ...
    save_to_db(FACTOR_CODE, SYMBOL, pub_date, obs_date, raw_value,
               source="COMEX Gold (GC) via AKShare", source_confidence=1.0)
