...
def main():
    ...
    print("(auto) === RU现货 === obs={}".format(obs_date))
    try:
        date_str = obs_date.strftime("%Y%m%d")
        df = ak.futures_spot_price(date=date_str, vars_list=["RU"])
        if df is not None and len(df) > 0:
            row = df.iloc[-1]
            val = float(row["spot_price"])
            save_to_db("RU_POS_NET", SYMBOL, pub_date, obs_date, val, source_confidence=1.0, source="akshare_futures_spot_price")
