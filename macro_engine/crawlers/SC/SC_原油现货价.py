...
"""
SC_原油现货价.py
因子: SC_SPT_SPOT = 原油现货价（元/桶）

公式: 直接取 AKShare futures_spot_price 的 spot_price 字段

当前状态: [✅正常]
- L1: AKShare futures_spot_price(date, vars_list=["SC"]) → spot_price
- L2: AKShare futures_main_sina(symbol="SC0") 收盘价作为现货代理
...
FCODE = "SC_SPT_SPOT"
SYM = "SC"
BOUNDS = (200.0, 1000.0)  # 原油现货价合理范围（元/桶）
BACKOFF_DAYS = 15

SPOT_SYM_ALTERNATIVES = ["SC", "sc", "原油", "原油期货"]
...
def fetch_l2(obs_date):
    """L2: futures_main_sina 收盘价作为现货代理"""
    try:
        df = ak.futures_main_sina(symbol="SC0")
        if df is not None and len(df) > 0:
            latest = df.sort_values('日期').iloc[-1]
            close = float(latest['收盘价'])
            print(f"  [L2] futures_main_sina: close={close:.2f}")
