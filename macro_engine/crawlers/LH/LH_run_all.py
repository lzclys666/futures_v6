...
def main():
    ...
    sep = "=" * 50
    print(sep)
    print(f"LH @ {time.strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    print(sep)
    total = ok + fail + skip
    print(f"LH Done {ok+fail+skip}/{total}  OK={ok} FAIL={fail} SKIP={skip}")
    print(sep)

    import datetime
    log_file = LOG_DIR / f"{datetime.date.today()}.log"
    with open(log_file, 'a', encoding='utf-8') as f:
        f.write(f"LH @ {datetime.datetime.now()} | pub={pub_date} obs={obs_date}\n")
