      ...
      <Row gutter={[16, 16]} style={{ marginBottom: 16 }}>
        ...
        <Col span={6}>
          <Card size="small">
            <Statistic
              ...
              precision={0}
              valueStyle={{
                color: (portfolio?.daily_pnl ?? 0) >= 0 ? '#52c41a' : '#ff4d4f',
              }}
              prefix={(portfolio?.daily_pnl ?? 0) >= 0 ? <ArrowUpOutlined /> : <ArrowDownOutlined />}
            />
          </Card>
        </Col>
        <Col span={6}>
          <Card size="small">
            <Statistic
              title="当日收益率"
              value={(portfolio?.daily_return ?? 0) * 100}
              precision={2}
              suffix="%"
              valueStyle={{
                color: (portfolio?.daily_return ?? 0) >= 0 ? '#52c41a' : '#ff4d4f',
              }}
            />
          </Card>
        </Col>
        <Col span={6}>
          <Card size="small">
            <Statistic
              title="持仓占比"
              value={portfolio?.total_position_pct ?? 0}
              precision={1}
              suffix="%"
            />
          </Card>
      ...
      <Card
        title={
          <Space>
            ...
            {riskStatus && (
              <Badge
                status={RISK_STATUS_MAP[riskStatus.overall_status]}
                text={riskStatus.overall_status}
              />
            )}
          </Space>
        }
