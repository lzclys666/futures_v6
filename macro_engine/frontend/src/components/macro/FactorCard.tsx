...
const FactorCard: React.FC<FactorCardProps> = ({ factor }) => {
  ...
  return (
    <Card size="small" className="factor-card" bordered={false}>
      ...
      {factor.factorIc !== undefined && (
        <div className="factor-card__ic">
          <span className="factor-card__ic-label">IC</span>
          <span className="factor-card__ic-value">
            {factor.factorIc > 0.1 ? '📈' : factor.factorIc < -0.1 ? '📉' : '➖'}
            <span
              className="factor-card__ic-number"
              style={{
                color: factor.factorIc > 0.1 ? '#52c41a' : factor.factorIc < -0.1 ? '#ff4d4f' : '#999999'
              }}
            >
              {factor.factorIc.toFixed(3)}
            </span>
          </span>
        </div>
      )}
